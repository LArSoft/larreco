  ...
  void CosmicTracker::produce(art::Event& evt){
    ...
    for (int i = 0; i<nplanes; ++i){
      for (size_t c1 = 0; c1<Cls[i].size(); ++c1){
        bool hasNonNullIntegral1 = signals[i][c1]->Integral();
        int Cls1 = Cls[i][c1];
        for (int j = 0; j<nplanes; ++j){
          for (size_t c2 = 0; c2<Cls[j].size(); ++c2){
            bool hasNonNullIntegral2 = signals[j][c2]->Integral();
            int Cls2 = Cls[j][c2];
            
            // check if both are the same view
            if (clusterlist[Cls1]->View()==
                clusterlist[Cls2]->View()) continue;
            // check if both are in the same cryostat and tpc
            if (clusterlist[Cls1]->Plane().Cryostat!=
                clusterlist[Cls2]->Plane().Cryostat) continue;
            if (clusterlist[Cls1]->Plane().TPC!=
                clusterlist[Cls2]->Plane().TPC) continue;
            // check if both are already in the matched list
            if (matched[Cls1]==1&&matched[Cls2]==1) continue;
            // KS test between two views in time
            double ks = 0;
            if (hasNonNullIntegral1 && hasNonNullIntegral2)
              ks = signals[i][c1]->KolmogorovTest(signals[j][c2]);
            else{
              mf::LogWarning("CosmicTracker") <<"One of the two clusters appears to be empty: "<<clusterlist[Cls1]->ID()<<" "<<clusterlist[Cls2]->ID();
            }
            hks->Fill(ks);
            ...
            if (ks>fKScut){//pass KS test
              ...
              for (size_t l = 0; l<matchedclusters.size(); ++l){
                for (size_t m = 0; m<matchedclusters[l].size(); ++m){
                  if (matchedclusters[l][m]==Cls1){
                    imatch = l; //track candidate
                    iadd = j; //consider the other cluster
                  }
                  else if (matchedclusters[l][m]==Cls2){
                    imatch = l; //track candidate
                ...
                if (iadd == i){
                  ...
                  for (size_t ii = 0; ii<matchedclusters[imatch].size(); ++ii){
                    if (clusterlist[matchedclusters[imatch][ii]]->View()==
                        clusterlist[Cls1]->View()){
                      matchview = true;
                      //replace if the new cluster has more hits
                      if (fm.at(Cls1).size()>fm.at(matchedclusters[imatch][ii]).size()){
                        matched[matchedclusters[imatch][ii]] = 0;
                        matchedclusters[imatch][ii] = Cls1;
                        matched[Cls1] = 1;
                      }
                    }
                  }
                  if (!matchview){//not matched view found, just add
                    matchedclusters[imatch].push_back(Cls1);
                    matched[Cls1] = 1;
                  }
                }
                else {
                  ...
                  for (size_t jj = 0; jj<matchedclusters[imatch].size(); ++jj){
                    if (clusterlist[matchedclusters[imatch][jj]]->View()==
                        clusterlist[Cls2]->View()){
                      matchview = true;
                      //replace if it has more hits
                      if (fm.at(Cls2).size()>fm.at(matchedclusters[imatch][jj]).size()){
                        matched[matchedclusters[imatch][jj]] = 0;
                        matchedclusters[imatch][jj] = Cls2;
                        matched[Cls2] = 1;
                      }
                    }
                  }
                  if (!matchview){
                    matchedclusters[imatch].push_back(Cls2);
                    matched[Cls2] = 1;
                  }
                }
              }
              else{
                std::vector<int> tmp{Cls1, Cls2};
                matchedclusters.push_back(std::move(tmp));
                matched[Cls1]=1;
                matched[Cls2]=1;
              }
            }//pass KS test
        ...
        for (auto itime1 = vtimemap[iclu1].begin(); 
          ...
          if (vtimemap[iclu1].size()==1){
            ...
            for (auto iw1 = vtimemap[iclu1].begin(); iw1!=itime1; ++iw1){
              auto iw2 = iw1;
              ++iw2;
              length1 += std::sqrt(sqr((iw1->first-iw2->first)*wire_pitch)
                                   +sqr((iw1->second-iw2->second)*timepitch));
            }
          }
          ...
          for (auto itime2 = vtimemap[iclu2].begin(); 
            ...
            if (vtimemap[iclu2].size()==1){
              ...
              auto iw2 = vtimemap[iclu2].begin(), iw1 = iw2;
              while (iw1 != itime2) {
                ++iw2;
                length2 += std::sqrt(sqr((iw1->first-iw2->first)*wire_pitch)+sqr((iw1->second-iw2->second)*timepitch));
                iw1 = iw2;
              } // while
            }
