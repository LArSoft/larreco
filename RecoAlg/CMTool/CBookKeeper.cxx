...
namespace cluster {
  ...
  void CBookKeeper::ProhibitMerge(unsigned short index1, unsigned short index2)
  {
    if(index1 == index2)
      ...
      throw CRUException(Form("<<%s>> Two input clusters identical (%d)",__FUNCTION__,index1));


    if( index1 >= this->size() || index2 >= this->size() )

      throw CRUException(Form("Input cluster index (%d and/or %d) out of range",index1,index2));

    auto out_index1 = this->at(index1);
    auto out_index2 = this->at(index2);

    if(out_index1 == out_index2)
      throw CRUException(Form("Cluster %d and %d already merged!",index1,index2));    
  ...
  void CBookKeeper::Merge(unsigned short index1, unsigned short index2)
  {
    ...
    if(_prohibit_merge.at(out_index1).at(out_index2-out_index1))
      ...
      throw CRUException(Form("Clusters (%d,%d) correspond to output (%d,%d) which is prohibited to merge",
				   index1,index2,
  ...
  { 
    if( index1 >= this->size() || index2 >= this->size() )
      throw CRUException(Form("Invalid cluster index: %d or %d",index1,index2));
  ...
  std::vector<unsigned short> CBookKeeper::GetMergedSet(unsigned short index1) const
  {
    ...
    if( index1 >= this->size() )
      throw CRUException(Form("Invalid cluster index: %d ",index1));
