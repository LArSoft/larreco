  ...
  std::map<int, std::vector<int>>& trackToClusters) const
{
  ...
  this->AssociateClustersAndTracks(
  ...
  std::map<int, std::vector<int>>& trackToClusters) const
{
  ...
  for (std::vector<art::Ptr<recob::Cluster>>::const_iterator clusterIt = clusters.begin();
    ...
    for (std::vector<art::Ptr<recob::Hit>>::iterator clusterHitIt = clusterHits.begin();
         clusterHitIt != clusterHits.end();
         ++clusterHitIt) {
      ...
      if (clusterHitTracks.at(0)->Length() < fMinTrackLength) {
        if (fDebug > 2)
          std::cout << "Track " << clusterHitTracks.at(0)->ID() << " is too short! ("
                    << clusterHitTracks.at(0)->Length() << ")" << std::endl;
        continue;
      }
      ...
      if (std::find(clustersToIgnore.begin(), clustersToIgnore.end(), cluster) !=
          clustersToIgnore.end())
        continue;
      if (std::find(trackToClusters[track].begin(), trackToClusters[track].end(), cluster) ==
          trackToClusters[track].end())
        trackToClusters[track].push_back(cluster);
      if (std::find(clusterToTracks[cluster].begin(), clusterToTracks[cluster].end(), track) ==
          clusterToTracks[cluster].end())
...
void
shower::EMShowerAlg::CheckIsolatedHits(
  std::map<int, std::vector<art::Ptr<recob::Hit>>>& showerHitsMap) const
{
  ...
  int problemPlane = -1;
  for (std::map<int, std::vector<int>>::iterator firstTPCIt = firstTPC.begin();
       firstTPCIt != firstTPC.end();
       ++firstTPCIt)
    if (firstTPCIt->second.size() == 1) problemPlane = firstTPCIt->second.at(0);
  ...
  if (tpcCount.size() == 1 and
    ...
    for (std::vector<art::Ptr<recob::Hit>>::iterator naughty_hitIt = naughty_hits.begin();
         naughty_hitIt != naughty_hits.end();
         ++naughty_hitIt)
...
bool
shower::EMShowerAlg::CheckShowerHits(
  std::map<int, std::vector<art::Ptr<recob::Hit>>> const& showerHitsMap) const
{
  ...
  if (showerHitsMap.size() < 2)
    ...
    double projectionDifference = ((HitPosition(startHits.at(0)) - proj1).Mod() +
    ...
    if (fDebug > 1)
      std::cout << "Timing difference is " << timingDifference << " and projection distance is "
                ...
                << showerStartPos.Y() << ", " << showerStartPos.Z() << ")" << std::endl;
  }
    ...
    for (int plane = 0; plane < 3; ++plane) {
      ...
      if (fDebug > 2) {
        ...
        std::cout << "Start position in this plane is " << HitPosition(start2DMap.at(plane)).X()
                  << ", " << HitPosition(start2DMap.at(plane)).Y() << ")" << std::endl;
        ...
        std::cout << "Projecting the other two planes gives position (" << showerStartProj.X()
                  << ", " << showerStartProj.Y() << ")" << std::endl;
      }

      double projDiff = TMath::Abs((showerStartProj - HitPosition(start2DMap.at(plane))).Mod());
      double timeDiff = TMath::Max(
        TMath::Abs(start2DMap.at(plane)->PeakTime() - start2DMap.at(otherPlanes.at(0))->PeakTime()),
        TMath::Abs(start2DMap.at(plane)->PeakTime() -
  ...
  if (fDebug > 1) std::cout << "Consistency check is " << consistencyCheck << std::endl;
                                       ...
                                       art::FindManyP<recob::Hit> const& fmh) const
{
  ...
  // Look at each shower
  for (std::vector<std::vector<int>>::const_iterator initialShowerIt = initialShowers.begin();
       initialShowerIt != initialShowers.end();
       ++initialShowerIt) {

    if (std::distance(initialShowers.begin(), initialShowerIt) > 0) continue;

    // Map the clusters and cluster hits to each view
    std::map<int, std::vector<art::Ptr<recob::Cluster>>> planeClusters;
    std::map<int, std::vector<art::Ptr<recob::Hit>>> planeHits;
    for (std::vector<int>::const_iterator clusterIt = initialShowerIt->begin();
         clusterIt != initialShowerIt->end();
         ++clusterIt) {
      art::Ptr<recob::Cluster> cluster = clusters.at(*clusterIt);
      std::vector<art::Ptr<recob::Hit>> hits = fmh.at(cluster.key());
      planeClusters[cluster->Plane().Plane].push_back(cluster);
      for (std::vector<art::Ptr<recob::Hit>>::iterator hitIt = hits.begin(); hitIt != hits.end();
           ++hitIt)
    ...
    for (std::map<int, std::vector<art::Ptr<recob::Cluster>>>::iterator planeIt =
           planeClusters.begin();
         planeIt != planeClusters.end();
         ++planeIt) {
      for (std::vector<art::Ptr<recob::Cluster>>::iterator clusterIt = planeIt->second.begin();
           clusterIt != planeIt->second.end();
           ++clusterIt) {
        chargeDist[planeIt->first] =
          new TH1D(std::string("chargeDist_Plane" + std::to_string(planeIt->first) + "_Cluster" +
                               std::to_string(clusterIt->key()))
                     .c_str(),
                   "",
                   150,
                   0,
    ...
    for (std::map<int, std::vector<double>>::iterator planeClusterSizesIt =
           planeClusterSizes.begin();
         planeClusterSizesIt != planeClusterSizes.end();
         ++planeClusterSizesIt) {
      double average = 0;
      for (std::vector<double>::iterator planeClusterSizeIt = planeClusterSizesIt->second.begin();
           planeClusterSizeIt != planeClusterSizesIt->second.end();
           ++planeClusterSizeIt)
    ...
    int badPlane = -1;
    for (std::map<int, double>::iterator clusterAvSizeIt = planeClustersAvSizes.begin();
         clusterAvSizeIt != planeClustersAvSizes.end();
         ++clusterAvSizeIt) {

      // Get averages from other planes and add in quadrature
      std::vector<double> otherAverages;
      for (std::map<int, double>::iterator otherClustersAvSizeIt = planeClustersAvSizes.begin();
           otherClustersAvSizeIt != planeClustersAvSizes.end();
           ++otherClustersAvSizeIt)
      ...
      for (std::vector<double>::iterator otherAvIt = otherAverages.begin();
           otherAvIt != otherAverages.end();
           ++otherAvIt)
      ...
      if (clusterAvSizeIt->second >= quadOtherPlanes) badPlane = clusterAvSizeIt->first;
    }

    if (badPlane != -1) {
      if (fDebug > 1) std::cout << "Bad plane is " << badPlane << std::endl;
      for (std::vector<art::Ptr<recob::Cluster>>::iterator clusterIt =
             planeClusters.at(badPlane).begin();
           clusterIt != planeClusters.at(badPlane).end();
           ++clusterIt)
...
TVector3
shower::EMShowerAlg::Construct3DPoint(art::Ptr<recob::Hit> const& hit1,
                                      art::Ptr<recob::Hit> const& hit2) const
{

  // x is average of the two x's
  double x = (fDetProp->ConvertTicksToX(hit1->PeakTime(), hit1->WireID().planeID()) +
...
shower::EMShowerAlg::ConstructTrack(std::vector<art::Ptr<recob::Hit>> const& hits1,
                                    ...
                                    std::map<int, TVector2> const& showerCentreMap) const
{
  ...
  if (fDebug > 1) {
    ...
    for (std::vector<art::Ptr<recob::Hit>>::const_iterator hit1 = track1.begin();
         hit1 != track1.end();
         ++hit1)
      std::cout << "Hit (" << HitCoordinates(*hit1).X() << ", " << HitCoordinates(*hit1).Y()
                << ") (real wire " << (*hit1)->WireID().Wire << ") in TPC " << (*hit1)->WireID().TPC
                << std::endl;
    for (std::vector<art::Ptr<recob::Hit>>::const_iterator hit2 = track2.begin();
         hit2 != track2.end();
         ++hit2)
      std::cout << "Hit (" << HitCoordinates(*hit2).X() << ", " << HitCoordinates(*hit2).Y()
                << ") (real wire " << (*hit2)->WireID().Wire << ") in TPC " << (*hit2)->WireID().TPC
                << std::endl;
  }
...
std::unique_ptr<recob::Track>
shower::EMShowerAlg::ConstructTrack(std::vector<art::Ptr<recob::Hit>> const& track1,
...
double
shower::EMShowerAlg::FinddEdx(std::vector<art::Ptr<recob::Hit>> const& trackHits,
                              std::unique_ptr<recob::Track> const& track) const
{
  ...
  for (std::vector<art::Ptr<recob::Hit>>::const_iterator trackHitIt = trackHits.begin();
  ...
  int plane) const
{
  ...
  if (fDebug > 1) {
    ...
    for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator initialTrackHitsIt =
           initialTrackHits.begin();
         initialTrackHitsIt != initialTrackHits.end();
         ++initialTrackHitsIt) {
      std::cout << "  Plane " << initialTrackHitsIt->first << std::endl;
      for (std::vector<art::Ptr<recob::Hit>>::iterator initialTrackHitIt =
             initialTrackHitsIt->second.begin();
           initialTrackHitIt != initialTrackHitsIt->second.end();
           ++initialTrackHitIt)
        std::cout << "    Hit is (" << HitCoordinates(*initialTrackHitIt).X() << " (real hit "
                  << (*initialTrackHitIt)->WireID() << "), "
                  << HitCoordinates(*initialTrackHitIt).Y() << ")" << std::endl;
    }
  }
  if (fDebug > 1)
...
shower::EMShowerAlg::FindOrderOfHits(std::vector<art::Ptr<recob::Hit>> const& hits,
                                     bool perpendicular) const
{
  ...
  for (std::vector<art::Ptr<recob::Hit>>::const_iterator hit = hits.begin(); hit != hits.end();
       ++hit) {
  ...
  std::transform(hitProjection.begin(),
                 hitProjection.end(),
  ...
  if (fMakeGradientPlot) {
    ...
    for (std::vector<art::Ptr<recob::Hit>>::iterator hitIt = showerHits.begin();
         hitIt != showerHits.end();
         ++hitIt) {
    ...
    for (std::map<int, TGraph*>::iterator graphIt = graphs.begin(); graphIt != graphs.end();
         ++graphIt) {
...
shower::EMShowerAlg::FindShowers(std::map<int, std::vector<int>> const& trackToClusters) const
{
  ...
  for (std::map<int, std::vector<int>>::const_iterator trackToClusterIt = trackToClusters.begin();
       trackToClusterIt != trackToClusters.end();
       ++trackToClusterIt) {
    ...
    for (unsigned int shower = 0; shower < showers.size(); ++shower)
      for (std::vector<int>::const_iterator cluster = trackToClusterIt->second.begin();
           cluster != trackToClusterIt->second.end();
           ++cluster)
        if ((std::find(showers.at(shower).begin(), showers.at(shower).end(), *cluster) !=
             showers.at(shower).end()) and
            (std::find(matchingShowers.begin(), matchingShowers.end(), shower)) ==
              matchingShowers.end())
    ...
    // New shower
    if (matchingShowers.size() < 1) showers.push_back(trackToClusterIt->second);
    ...
    // Add to existing shower
      ...
      for (std::vector<int>::const_iterator cluster = trackToClusterIt->second.begin();
           cluster != trackToClusterIt->second.end();
           ++cluster)
        if (std::find(showers.at(matchingShowers.at(0)).begin(),
                      showers.at(matchingShowers.at(0)).end(),
                      *cluster) == showers.at(matchingShowers.at(0)).end())
  ...
  std::map<int, std::vector<art::Ptr<recob::Hit>>> const& orderedShowerMap) const
{
  ...
  for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::const_iterator orderedShowerIt =
         orderedShowerMap.begin();
       orderedShowerIt != orderedShowerMap.end();
       ++orderedShowerIt) {
    ...
    if (wireDirection) {
      bool increasing =
        HitCoordinates(*orderedShower.rbegin()).X() > HitCoordinates(*orderedShower.begin()).X();
    ...
    // Deal with showers travelling along ticks
      ...
      bool increasing =
        HitCoordinates(*orderedShower.rbegin()).Y() > HitCoordinates(*orderedShower.begin()).Y();
    ...
    for (std::vector<art::Ptr<recob::Hit>>::iterator initialHitIt = initialHits.begin();
         initialHitIt != initialHits.end();
         ++initialHitIt)
    ...
    if (singleHitTPCs.size()) {
      if (fDebug > 2)
        for (std::vector<int>::iterator tpcIt = singleHitTPCs.begin(); tpcIt != singleHitTPCs.end();
             ++tpcIt)
          std::cout << "Removed hits in TPC " << *tpcIt << std::endl;
      for (std::vector<art::Ptr<recob::Hit>>::iterator initialHitIt = initialHits.begin();
           initialHitIt != initialHits.end();
           ++initialHitIt)
        if (std::find(singleHitTPCs.begin(), singleHitTPCs.end(), (*initialHitIt)->WireID().TPC) ==
            singleHitTPCs.end())
...
shower::EMShowerAlg::GetPlanePermutations(
  const std::map<int, std::vector<art::Ptr<recob::Hit>>>& showerHitsMap) const
{
  ...
  if (fDebug > 1)
    for (std::map<double, int>::const_iterator wireWidthIt = wireWidthMap.begin();
         wireWidthIt != wireWidthMap.end();
         ++wireWidthIt)
      std::cout << "Plane " << wireWidthIt->second << " has relative width in wire of "
                << wireWidthIt->first << "; and an RMS gradient of "
                << planeRMSGradients[wireWidthIt->second] << std::endl;
  ...
  for (std::map<double, int>::iterator wireWidthIt = wireWidthMap.begin();
       wireWidthIt != wireWidthMap.end();
       ++wireWidthIt) {
    std::map<int, bool> permutation;
    permutation[wireWidthIt->second] = 1;
    for (std::map<double, int>::iterator wireWidth2It = wireWidthMap.begin();
         wireWidth2It != wireWidthMap.end();
         ++wireWidth2It)
      if (wireWidthIt->second != wireWidth2It->second) permutation[wireWidth2It->second] = 0;
    if (std::find(usedPermutations.begin(), usedPermutations.end(), permutation) ==
        usedPermutations.end()) {
      permutations[perm] = permutation;
      usedPermutations.push_back(permutation);
      ++perm;
    }
  }
  for (std::map<double, int>::reverse_iterator wireWidthIt = wireWidthMap.rbegin();
       wireWidthIt != wireWidthMap.rend();
       ++wireWidthIt) {
    std::map<int, bool> permutation;
    ...
    for (std::map<double, int>::iterator wireWidth2It = wireWidthMap.begin();
         wireWidth2It != wireWidthMap.end();
         ++wireWidth2It)
      if (wireWidthIt->second != wireWidth2It->second) permutation[wireWidth2It->second] = 1;
    if (std::find(usedPermutations.begin(), usedPermutations.end(), permutation) ==
        usedPermutations.end()) {
      permutations[perm] = permutation;
      usedPermutations.push_back(permutation);
  ...
  if (fDebug > 1) {
    ...
    for (std::map<int, std::map<int, bool>>::iterator permIt = permutations.begin();
         permIt != permutations.end();
         ++permIt) {
      std::cout << "Permutation " << permIt->first << std::endl;
      for (std::map<int, bool>::iterator planePermIt = permIt->second.begin();
           planePermIt != permIt->second.end();
           ++planePermIt)
        std::cout << "  Plane " << planePermIt->first << " has value " << planePermIt->second
                  << std::endl;
    }
  }
...
std::unique_ptr<recob::Track>
shower::EMShowerAlg::MakeInitialTrack(
  ...
  std::map<int, std::vector<art::Ptr<recob::Hit>>> const& initialHitsMap) const
{
  ...
  if (fDebug > 0) {
    ...
    std::cout << "dE/dx for each plane is: " << dEdx[0] << ", " << dEdx[1] << " and " << dEdx[2]
              << std::endl;
    std::cout << "Total energy for each plane is: " << totalEnergy[0] << ", " << totalEnergy[1]
              << " and " << totalEnergy[2] << std::endl;
    std::cout << "The shower start is (" << showerStart.X() << ", " << showerStart.Y() << ", "
              << showerStart.Z() << ")" << std::endl;
                                ...
                                int& iok) const
{
  ...
  if (pl0 != -1 && pl1 != -1 && initialTrackHits[pl0].size() >= 2 &&
    ...
    pma::Track3D* pmatrack =
      fProjectionMatchingAlg.buildSegment(initialTrackHits[pl0], initialTrackHits[pl1]);
    ...
    if (spts.size() >= 2) { //at least two space points
      ...
      if (fDebug > 1) {
        ...
        std::cout << "Total energy for each plane is: " << totalEnergy[0] << ", " << totalEnergy[1]
                  << " and " << totalEnergy[2] << std::endl;
        std::cout << "The shower start is (" << shwxyz.X() << ", " << shwxyz.Y() << ", "
                  << shwxyz.Z() << ")" << std::endl;
        shwxyz.Print();
      }
...
shower::EMShowerAlg::MakeSpacePoints(std::map<int, std::vector<art::Ptr<recob::Hit>>> showerHits,
  ...
  for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::const_iterator showerHitIt =
    ...
    for (std::vector<art::Ptr<recob::Hit>>::const_iterator planeHitIt = showerHitIt->second.begin();
      ...
      for (std::vector<art::Ptr<recob::Hit>>::const_iterator otherPlaneHitIt =
        ...
        else if ((Project3DPointOntoPlane(point, (*planeHitIt)->WireID().Plane) -
                  HitPosition(*planeHitIt))
                     .Mod() < fSpacePointSize and
                 (Project3DPointOntoPlane(point, (*otherPlaneHitIt)->WireID().Plane) -
                  HitPosition(*otherPlaneHitIt))
                     .Mod() < fSpacePointSize) {
...
std::map<int, std::vector<art::Ptr<recob::Hit>>>
shower::EMShowerAlg::OrderShowerHits(art::PtrVector<recob::Hit> const& shower, int plane) const
{
  ...
  for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator showerHitsIt =
       ...
       showerHitsIt != showerHitsMap.end();
       ++showerHitsIt) {
    if (plane != showerHitsIt->first and plane != -1) continue;
  ...
  if (fDebug > 1)
    for (std::map<int, double>::iterator planeRMSIt = planeRMS.begin();
         ...
         ++planeRMSIt)
      std::cout << "Plane " << planeRMSIt->first << " has RMS " << planeRMSIt->second
  ...
  if (fDebug > 2) {
    ...
    for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator showerHitsIt =
      ...
      for (std::vector<art::Ptr<recob::Hit>>::iterator hitIt = showerHitsIt->second.begin();
           ...
           ++hitIt)
        std::cout << "    Hit at (" << HitCoordinates(*hitIt).X() << ", "
                  << HitCoordinates(*hitIt).Y() << ") -- real wire " << (*hitIt)->WireID()
                  << ", hit position (" << HitPosition(*hitIt).X() << ", "
                  << HitPosition(*hitIt).Y() << ")" << std::endl;
    }
  }
  ...
  for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator showerHitsIt =
    ...
    if (planeRMS.at(showerHitsIt->first) > planeOtherRMS.at(showerHitsIt->first) * 2) {
      ...
      if (fDebug > 1)
        std::cout << "Plane " << showerHitsIt->first << " was perpendicular... recalculating"
                  << std::endl;
      std::vector<art::Ptr<recob::Hit>> orderedHits =
        this->FindOrderOfHits(showerHitsIt->second, true);
      showerHitsMap[showerHitsIt->first] = orderedHits;
  ...
  if (fDebug > 1) {
    ...
    for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator showerHitsIt =
      ...
      std::cout << "  Plane " << showerHitsIt->first << " has start ("
                << HitCoordinates(showerHitsIt->second.front()).X() << ", "
                << HitCoordinates(showerHitsIt->second.front()).Y() << ") (real wire "
                << showerHitsIt->second.front()->WireID() << ") and end ("
                << HitCoordinates(showerHitsIt->second.back()).X() << ", "
                << HitCoordinates(showerHitsIt->second.back()).Y() << ") (real wire "
                << showerHitsIt->second.back()->WireID() << ")" << std::endl;
  }
  ...
  if (fDebug > 1) {
    std::cout << "After reversing and checking isolated hits, here are the start and end points..."
              << std::endl;
    for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator showerHitsIt =
           showerHitsMap.begin();
         showerHitsIt != showerHitsMap.end();
      ...
      std::cout << "  Plane " << showerHitsIt->first << " has start ("
                ...
                << showerHitsIt->second.front()->WireID() << ") and end ("
                << HitCoordinates(showerHitsIt->second.back()).X() << ", "
                << HitCoordinates(showerHitsIt->second.back()).Y() << ")" << std::endl;
  }
  ...
  if (fDebug > 1) std::cout << "Here are the relative wire widths... " << std::endl;
  for (std::map<double, int>::iterator wireWidthIt = wireWidths.begin();
       wireWidthIt != wireWidths.end();
       ++wireWidthIt) {
    if (fDebug > 1)
      std::cout << "Plane " << wireWidthIt->second << " has relative wire width "
                << wireWidthIt->first << std::endl;
  ...
  if (badPlanes.size() == 1) {
    ...
    if (fDebug > 1) std::cout << "Ignoring plane " << badPlane << " when orientating" << std::endl;
  ...
  while (!CheckShowerHits(showerHitsMap) and n < TMath::Power(2, (int)showerHitsMap.size())) {
    if (fDebug > 1) std::cout << "Permutation " << n << std::endl;
    for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator showerHitsIt =
           showerHitsMap.begin();
         showerHitsIt != showerHitsMap.end();
         ++showerHitsIt) {
  ...
  if (!CheckShowerHits(showerHitsMap)) showerHitsMap = originalShowerHitsMap;

  if (fDebug > 2) {
    std::cout << "End of OrderShowerHits: here are the order of hits:" << std::endl;
    for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator planeHitsIt =
           showerHitsMap.begin();
         planeHitsIt != showerHitsMap.end();
         ++planeHitsIt) {
      std::cout << "  Plane " << planeHitsIt->first << std::endl;
      for (std::vector<art::Ptr<recob::Hit>>::iterator hitIt = planeHitsIt->second.begin();
           hitIt != planeHitsIt->second.end();
           ++hitIt)
        std::cout << "    Hit (" << HitCoordinates(*hitIt).X() << " (real wire "
                  << (*hitIt)->WireID() << "), " << HitCoordinates(*hitIt).Y() << ") -- pos ("
                  << HitPosition(*hitIt).X() << ", " << HitPosition(*hitIt).Y() << ")" << std::endl;
    }
  }
...
void
shower::EMShowerAlg::OrderShowerHits(std::vector<art::Ptr<recob::Hit>> const& shower,
                                     std::vector<art::Ptr<recob::Hit>>& showerHits,
...
TVector2
shower::EMShowerAlg::HitCoordinates(art::Ptr<recob::Hit> const& hit) const
{
...
TVector2
shower::EMShowerAlg::HitPosition(art::Ptr<recob::Hit> const& hit) const
{
...
TVector2
shower::EMShowerAlg::HitPosition(TVector2 const& pos, geo::PlaneID planeID) const
{

  return TVector2(pos.X() * fGeom->WirePitch(planeID), fDetProp->ConvertTicksToX(pos.Y(), planeID));
}

double
shower::EMShowerAlg::GlobalWire(const geo::WireID& wireID) const
{
...
std::map<double, int>
shower::EMShowerAlg::RelativeWireWidth(
  const std::map<int, std::vector<art::Ptr<recob::Hit>>>& showerHitsMap) const
{
  ...
  for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::const_iterator showerHitsIt =
    ...
    planeWireLength[showerHitsIt->first] =
      TMath::Abs(HitCoordinates(showerHitsIt->second.front()).X() -
                 HitCoordinates(showerHitsIt->second.back()).X());
...
TVector2
shower::EMShowerAlg::ShowerDirection(const std::vector<art::Ptr<recob::Hit>>& showerHits) const
{
...
TVector2
shower::EMShowerAlg::ShowerCentre(const std::vector<art::Ptr<recob::Hit>>& showerHits) const
{
...
double
shower::EMShowerAlg::ShowerHitRMS(const std::vector<art::Ptr<recob::Hit>>& showerHits) const
{
...
double
shower::EMShowerAlg::ShowerHitRMSGradient(const std::vector<art::Ptr<recob::Hit>>& showerHits,
                                          TVector2 trueStart) const
{
  ...
  for (std::vector<art::Ptr<recob::Hit>>::const_iterator showerHitsIt = showerHits.begin();
       showerHitsIt != showerHits.end();
       ++showerHitsIt) {
    showerSegments[(int)(HitPosition(*showerHitsIt) - HitPosition(showerHits.front())).Mod() /
                   lengthOfSegment]
      .push_back(*showerHitsIt);
    segmentCharge[(int)(HitPosition(*showerHitsIt) - HitPosition(showerHits.front())).Mod() /
  ...
  // Loop over the bins to find the distribution of hits as the shower progresses
  for (std::map<int, std::vector<art::Ptr<recob::Hit>>>::iterator showerSegmentIt =
         showerSegments.begin();
    ...
    double RMSBin = TMath::RMS(distanceToAxisBin.begin(), distanceToAxisBin.end());
    binVsRMS.push_back(std::make_pair(showerSegmentIt->first, RMSBin));
    if (fMakeRMSGradientPlot) graph->SetPoint(graph->GetN(), showerSegmentIt->first, RMSBin);
  }

  // Get the gradient of the RMS-bin plot
  double sumx = 0., sumy = 0., sumx2 = 0., sumxy = 0., sumweight = 0.;
  for (std::vector<std::pair<int, double>>::iterator binVsRMSIt = binVsRMS.begin();
...
TVector2
shower::EMShowerAlg::Project3DPointOntoPlane(TVector3 const& point, int plane, int cryostat) const
{
  ...
  wireTickPos = TVector2(GlobalWire(wireID), fDetProp->ConvertXToTicks(point.X(), planeID));
  ...
  const std::map<int, std::vector<art::Ptr<recob::Hit>>>& showerHitsMap) const
{
  ...
  if (fDebug > 1)
    for (std::map<int, double>::const_iterator planeOtherWireLengthIt =
           planeOtherWireLengths.begin();
         planeOtherWireLengthIt != planeOtherWireLengths.end();
         ++planeOtherWireLengthIt)
      std::cout << "Plane " << planeOtherWireLengthIt->first << " has "
                << planeWireLength[planeOtherWireLengthIt->first]
                << " wire width and therefore has relative width in wire of "
                << planeOtherWireLengthIt->second << std::endl;
...
int
shower::EMShowerAlg::FindTrueParticle(const std::vector<art::Ptr<recob::Hit>>& showerHits) const
{
  ...
  for (std::vector<art::Ptr<recob::Hit>>::const_iterator showerHitIt = showerHits.begin();
       showerHitIt != showerHits.end();
       ++showerHitIt) {
  ...
  int showerTrack = 0;
  for (std::map<int, double>::iterator trackIt = trackMap.begin(); trackIt != trackMap.end();
       ++trackIt) {
...
int
shower::EMShowerAlg::FindParticleID(const art::Ptr<recob::Hit>& hit) const
{
