...
double
shower::ShowerEnergyAlg::ShowerEnergy(std::vector<art::Ptr<recob::Hit>> const& hits,
                                      int plane) const
{
  ...
  for (art::PtrVector<recob::Hit>::const_iterator hit = hits.begin(); hit != hits.end(); ++hit) {
    ...
    totalCharge += ((*hit)->Integral() * TMath::Exp((detprop->SamplingRate() * (*hit)->PeakTime()) /
                                                    (detprop->ElectronLifetime() * 1e3)));
  }

  switch (plane) {
  case 0: totalEnergy = (totalCharge * fUGradient) + fUIntercept; break;
  case 1: totalEnergy = (totalCharge * fVGradient) + fVIntercept; break;
  case 2: totalEnergy = (totalCharge * fZGradient) + fZIntercept; break;
  }
