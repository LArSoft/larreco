...
public:
  ...
  unsigned int FindClusters(std::vector<std::vector<double>> const& image,
  ...
  void SaveImage(TH2F* image, std::vector<std::vector<unsigned int>> const& allClusterBins, int pad, int tpc, int plane);
...
private:
  ...
  art::Ptr<recob::Hit> ConvertBinToRecobHit(std::vector<std::vector<double>> const& image, unsigned int bin) const;

  /// Converts an xbin and a ybin to a global bin number
  unsigned int ConvertWireTickToBin(std::vector<std::vector<double>> const& image,
                                    unsigned int xbin,
                                    unsigned int ybin) const;

  /// Returns the charge stored in the global bin value
  double ConvertBinToCharge(std::vector<std::vector<double>> const& image, unsigned int bin) const;
  ...
  std::pair<unsigned int, unsigned int> DeadWireCount(unsigned int wire_bin, unsigned int width) const;

  /// Dynamically find the blurring radii and Gaussian sigma in each dimension
  std::array<unsigned int, 4> FindBlurringParameters();
  ...
  unsigned int fBlurWire;                 // blur radius for Gauss kernel in the wire direction
  unsigned int fBlurTick;                 // blur radius for Gauss kernel in the tick direction
  ...
  unsigned int fMaxTickWidthBlur;         // maximum distance to blur a hit based on its natural width in time
  unsigned int fClusterWireDistance;      // how far to cluster from seed in wire direction
  unsigned int fClusterTickDistance;      // how far to cluster from seed in tick direction
  ...
  // Blurring stuff
  unsigned int fKernelWidth, fKernelHeight;
  ...
  unsigned int fLowerTick, fUpperTick;
  unsigned int fLowerWire, fUpperWire;
