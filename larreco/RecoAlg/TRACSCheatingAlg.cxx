...
shower::TRACSCheatingAlg::GetTrueParticleMap() const
{
...
shower::TRACSCheatingAlg::GetTrueChain(std::map<int, const simb::MCParticle*>& trueParticles) const
{
                                        ...
                                        const art::Ptr<recob::PFParticle>& pfparticle) const
{
  ...
  if (Event.getByLabel(fHitModuleLabel, hitHandle)) { art::fill_ptr_vector(hits, hitHandle); }
  ...
  for (auto spacePoint : showerSpacePoints) {
    ...
    double proj = fTRACSAlg.SpacePointProjection(spacePoint, showerStartPosition, showerDirection);
