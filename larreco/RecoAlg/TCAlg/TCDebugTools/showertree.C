...
void showertree::Loop() {
   ...
   for (Long64_t jentry = 0; jentry < nentries; jentry++) {
      ...
      struct stat sb;
      if (!(stat("tcs_sh_debug_output", &sb) == 0 && S_ISDIR(sb.st_mode))) {
         cout << "Creating folder 'tc_sh_debug_output'" << endl;
         mkdir("tc_sh_debug_output", 0777);
      }

      bothViews[0]->SaveAs(Form("tc_sh_debug_output/tjs_%d.pdf[", event));
      for (int i = 0; i < nstages; ++i) {
         bothViews[i]->SaveAs(Form("tc_sh_debug_output/tjs_%d.pdf", event));
      }
      bothViews[nstages-1]->SaveAs(Form("tc_sh_debug_output/tjs_%d.pdf]", event));
