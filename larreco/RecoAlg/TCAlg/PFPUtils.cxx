...
namespace tca {
                  ...
                  unsigned short matVec_Iter)
  {
    ...
    for (std::size_t indx = 0; indx < matVec.size(); ++indx) {
      ...
      if (!MakeTP3Ds(detProp, slc, pfpVec[0])) {
      ...
      for (unsigned short ip = 0; ip < pfpVec.size(); ++ip) {
        ...
        KillBadPoints(clockData, detProp, slc, pfp, 50., foundMVI);
        ...
        if (pfp.SectionFits[0].ChiDOF < tcc.match3DCuts[5]) {
          ...
          KillBadPoints(clockData, detProp, slc, pfp, tcc.match3DCuts[4], foundMVI);
        ...
        if(tcc.dbgPFP && pfp.MVI == debug.MVI) PrintTP3Ds(clockData, detProp, "STORE", slc, pfp, -1);
            ...
            bool prt)
  {
    ...
    if (pfp.TP3Ds.size() > 100) {
      ...
      FitTP3Ds(clockData, detProp, slc, pfp, fromPt, nhalf, USHRT_MAX, chiDOF);
    ...
    for (unsigned short sfIndex = 0; sfIndex < 20; ++sfIndex) {
      ...
      for(unsigned short nit = 0; nit < maxNit; ++nit) {
        ...
        if (!FitTP3Ds(clockData, detProp, slc, pfp, fromPt, nPts, USHRT_MAX, chiDOF)) {
  ...
  /////////////////////////////////////////
  void
  KillBadPoints(detinfo::DetectorClocksData const& clockData,
                detinfo::DetectorPropertiesData const& detProp,
                TCSlice& slc,
                PFPStruct& pfp,
                float pullCut,
             ...
             unsigned short sfIndex)
  {
    ...
    float chiDOF = 999;
    return FitTP3Ds(clockData, detProp, slc, pfp, fromPt, npts, sfIndex, chiDOF);
           ...
           float& chiDOF)
  {
    ...
    auto sf = FitTP3Ds(clockData, detProp, slc, pfp.TP3Ds, fromPt, 1, nPtsFit);
  ...
  /////////////////////////////////////////
  void
  KinkFit(detinfo::DetectorClocksData const& clockData,
          detinfo::DetectorPropertiesData const& detProp,
          const TCSlice& slc,
          const PFPStruct& pfp,
          unsigned short atPt,
          ...
          double& dangSig)
  {
    ...
    auto fitMinus = FitTP3Ds(clockData, detProp, slc, pfp.TP3Ds, atPt - 1, -1, nPtsMinus);
    if (fitMinus.ChiDOF > 10) return;
    auto fitPlus = FitTP3Ds(clockData, detProp, slc, pfp.TP3Ds, atPt + 1, 1, nPtsMinus);
  ...
  /////////////////////////////////////////
  bool
  MakeTP3Ds(detinfo::DetectorPropertiesData const& detProp, TCSlice& slc, PFPStruct& pfp)
  {
    ...
    for(auto tid : pfp.TjIDs) {
      ...
      if(MakeSmallAnglePFP(detProp, slc, pfp)) return true;
    } // tid
...
//    std::cout<<"P"<<pfp.ID<<" longTID "<<longTID<<" nextLongTID "<<nextLongTID<<"\n";
    ...
    // temp
    auto tp3d0 = MakeTP3D(detProp, slc, ltp0, nltp0);
    auto tp3d1 = MakeTP3D(detProp, slc, ltp1, nltp1);
    ...
    for(unsigned short ipt = ltj.EndPt[0]; ipt <= ltj.EndPt[1]; ++ipt) {
      auto tp3d = CreateTP3D(detProp, slc, longTID, ipt);
      tp3d.SFIndex = 0;
...
//        std::cout<<" pos "<<position<<" length "<<tp3ds.size()<<"\n";
                ...
                detinfo::DetectorPropertiesData const& detProp,TCSlice& slc, PFPStruct& pfp)
  {
    ...
    for(std::size_t ipt = 1; ipt < pfp.TP3Ds.size() - 1; ++ipt) {
      ...
      if(!tp3d.Flags[kTP3DGood] || tp3d.TPIndex == USHRT_MAX) continue;
      if(tp3d.along < 0) {
        sumNeg += dEdx(clockData, detProp, slc, tp3d);
        ++cntNeg;
      } else {
        sumPos += dEdx(clockData, detProp, slc, tp3d);
        ++cntPos;
      }
    } // ipt
             ...
             short printPts)
  {
    ...
    for (unsigned short ipt = fromPt; ipt <= toPt; ++ipt) {
      ...
      KinkFit(clockData, detProp, slc, pfp, ipt, tcc.kinkCuts[3], dang, dangSig);
