...
public:
     ...
     */
    float getTimeToExecute() const override {return std::accumulate(fTimeVector.begin(),fTimeVector.end(),0.);}
...
private:
    ...
    std::unique_ptr<lar_cluster3d::IClusterParametersBuilder> fClusterBuilder;        ///<  Common cluster builder tool
};
...
void MSTPathFinder::configure(fhicl::ParameterSet const &pset)
{
    ...
    fClusterBuilder = art::make_tool<lar_cluster3d::IClusterParametersBuilder>(pset.get<fhicl::ParameterSet>("ClusterParamsBuilder"));
...
void MSTPathFinder::ModifyClusters(reco::ClusterParametersList& clusterParametersList) const
{
    ...
    for(auto& clusterParams : clusterParametersList)
    {
        ...
        if (clusterParams.getHitPairListPtr().size() > fMinTinyClusterSize)
        {
            ...
            kdTree::KdTreeNode     topNode = fkdTree.BuildKdTree(clusterParams.getHitPairListPtr(), kdTreeNodeContainer);
            ...
            for(auto& daughterParams : daughterParametersList) FindBestPathInCluster(daughterParams, topNode);
        }
    }
                                      ...
                                      reco::ClusterParametersList& clusterParametersList) const
{
    ...
    while(1)
    {
        ...
        {
            std::cout << "-----------------------------------------------------------------------------------------" << std::endl;
            std::cout << "**> Cluster idx: " << clusterIdx++ << " has " << curClusterHitList->size() << " hits" << std::endl;
...
void MSTPathFinder::FindBestPathInCluster(reco::ClusterParameters& clusterParams, kdTree::KdTreeNode& topNode) const
{
    ...
    if (clusterParams.getHitPairListPtr().size() > 2)
    {
        ...
        {
            std::cout << "++++++>>> PCA failure! # hits: " << clusterParams.getHitPairListPtr().size() << std::endl;
        }
    }
...
float MSTPathFinder::DistanceBetweenNodes(const reco::ClusterHit3D* node1,const reco::ClusterHit3D* node2) const
{
...
void MSTPathFinder::PruneAmbiguousHits(reco::ClusterParameters& clusterParams, reco::Hit2DToClusterMap& hit2DToClusterMap) const
{
...
void MSTPathFinder::buildConvexHull(reco::ClusterParameters& clusterParameters, reco::HitPairListPtr& hitPairListPtr, int level) const
{
...
float MSTPathFinder::findConvexHullEndPoints(const reco::EdgeList& convexHull, const reco::ClusterHit3D* first3D, const reco::ClusterHit3D* last3D) const
{
