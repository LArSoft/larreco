...
namespace cmtool {
  ...
  //-----------------------------
  void
  CFAlgoChargeDistrib::Reset()
  //-----------------------------
  {}

  //----------------------------------------------------------------------------------------------
  float
  CFAlgoChargeDistrib::Float(const std::vector<const cluster::ClusterParamsAlg*>& clusters)
  //----------------------------------------------------------------------------------------------
  {
    ...
    if (_verbose) {
      std::cout << "Number of clusters taken into account: " << clusters.size() << std::endl;
    }
    ...
    std::vector<std::vector<util::PxHit>> Hits; //(clusters.size(), std::vector<util::PxHit>());
  ...
  //------------------------------
  void
  CFAlgoChargeDistrib::Report()
  //------------------------------
  {}

  // Function to calculate the "convolution"
  float
  CFAlgoChargeDistrib::TProfConvol(std::vector<util::PxHit> hA, std::vector<util::PxHit> hB)
  {
