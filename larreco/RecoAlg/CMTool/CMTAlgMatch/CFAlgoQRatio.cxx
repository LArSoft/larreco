...
namespace cmtool {
  ...
  //-----------------------------------------------------------------------------------------
  float CFAlgoQRatio::Float(util::GeometryUtilities const&,
  ...
  //-----------------------------------------------------------------------------------------
  {
    ...
    for (auto const& c : clusters) {
      ...
      if (c->GetParams().sum_charge < 0) {
        if (_verbose)
          std::cout << "\033[00m Found a cluster with negative charge!\033[00m ... aborting "
                    << __FUNCTION__ << std::endl;
        return -1;
      }

      if (q_max < c->GetParams().sum_charge) q_max = c->GetParams().sum_charge;
    }
    ...
    if (_verbose) {
      if (ratio > _qratio_cut)
        ...
        std::cout << " ... below cut value: " << _qratio_cut << std::endl;
    }
