...
namespace cmtool {
  ...
  //-----------------------------
  void
  CFAlgoTimeProf::Reset()
  //-----------------------------
  {}

  //----------------------------------------------------------------------------------------------
  float
  CFAlgoTimeProf::Float(const std::vector<const cluster::ClusterParamsAlg*>& clusters)
  //----------------------------------------------------------------------------------------------
  {
    // We now have a vector a clusters.

    //### need a pointer to the cluster just return -1
    for (auto const& ptr : clusters)
      if (!ptr) return -1;
  ...
  float
  CFAlgoTimeProf::TProfCompare(std::vector<util::PxHit> hita, std::vector<util::PxHit> hitb)
  {
    ...
    double time_diff = (detp->GetXTicksOffset((int)(hita.at(0).plane), 0, 0) -
                        detp->GetXTicksOffset((int)(hitb.at(0).plane), 0, 0)) *
                       geou.TimeToCm();
    ...
    if (histo_inta.Integral())
      histo_inta.Scale(1. / histo_inta.GetBinContent(histo_inta.GetNbinsX()));
...
*/
    ...
    if (histo_intb.Integral())
      histo_intb.Scale(1. / histo_intb.GetBinContent(histo_intb.GetNbinsX()));
