...
namespace cmtool {
  ...
  void CMergeManager::IterationEnd()
  {
    ...
    if (_debug_mode <= kPerIteration) {
      ...
      std::cout << Form("  Input / Output cluster length: %zu/%zu",
                        _tmp_merged_clusters.size(),
                        _out_clusters.size())
                << std::endl;
      ...
      if (!_merge_till_converge) std::cout << "  Non-iterative option: terminating..." << std::endl;
    }
  ...
  bool CMergeManager::IterationProcess(util::GeometryUtilities const& gser)
  {
    ...
    if (!_iter_ctr)
      _tmp_merged_clusters = _in_clusters;
    else
      _tmp_merged_clusters = _out_clusters;
    _out_clusters.clear();
    ...
    if (_separate_algo) RunSeparate(_tmp_merged_clusters, bk);

    // Run merging algorithm
    RunMerge(_tmp_merged_clusters, merge_switch, bk);
    if (_debug_mode <= kPerIteration) _merge_algo->Report();
    ...
    if (_tmp_merged_clusters.size() == _out_clusters.size()) return false;

    if (_out_clusters.size() == _planes.size()) return false;
                               ...
                               CMergeBookKeeper& book_keeper) const
  {
    if (merge_flag.size() != in_clusters.size())
      throw CMTException(
        Form("in_clusters (%zu) and merge_flag (%zu) vectors must be of same length!",
             in_clusters.size(),
             merge_flag.size()));
    if (_debug_mode <= kPerIteration) {

      std::cout << Form("  Calling %s with %zu clusters...", __FUNCTION__, in_clusters.size())
                << std::endl;
    }
    ...
    for (auto citer1 = _priority.rbegin(); citer1 != _priority.rend(); ++citer1) {
      ...
      while (1) {
        ...
        if (_debug_mode <= kPerMerging) {
          ...
          std::cout << Form("    \033[93mInspecting a pair (%zu, %zu) for merging... \033[00m",
                            (*citer1).second,
                            (*citer2).second)
                    << std::endl;
        }
        ...
        bool merge =
          _merge_algo->Bool(in_clusters.at((*citer1).second), in_clusters.at((*citer2).second));

        if (_debug_mode <= kPerMerging) {

          if (merge)
            std::cout << "    \033[93mfound to be merged!\033[00m " << std::endl << std::endl;

          else
            std::cout << "    \033[93mfound NOT to be merged...\033[00m" << std::endl << std::endl;

        } // end looping over all sets of algorithms

        if (merge) book_keeper.Merge((*citer1).second, (*citer2).second);
    ...
    for (size_t cindex1 = 0; cindex1 < in_clusters.size(); ++cindex1) {
      ...
      for (size_t cindex2 = cindex1 + 1; cindex2 < in_clusters.size(); ++cindex2) {
        ...
        if (_debug_mode <= kPerMerging) {
          ...
          std::cout << Form("    \033[93mInspecting a pair (%zu, %zu) for separation... \033[00m",
                            cindex1,
                            cindex2)
                    << std::endl;
        }
        ...
        if (_debug_mode <= kPerMerging) {
          ...
          if (separate)
            std::cout << "    \033[93mfound to be separated!\033[00m " << std::endl << std::endl;

          else
            std::cout << "    \033[93mfound NOT to be separated...\033[00m" << std::endl
                      << std::endl;

        } // end looping over all sets of algorithms

        if (separate) book_keeper.ProhibitMerge(cindex1, cindex2);
