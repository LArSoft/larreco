...
namespace cmtool {
  ...
  CMManagerBase::SetClusters(const std::vector<cluster::ClusterParamsAlg>& clusters)
  {
    ...
    if (_time_report)
      std::cout << Form("  CMManagerBase Time Report: SetClusters (copy) = %g [s]",
                        localWatch.RealTime())
                << std::endl;
  }
  ...
  CMManagerBase::Process()
  {
    ...
    if (_time_report)
      std::cout << Form("  CMManagerBase Time Report: EventBegin = %g [s]", localWatch.RealTime())
                << std::endl;
    ...
    while (keep_going) {
      ...
      if (_time_report)
        std::cout << Form("  CMManagerBase Time Report: IterationBegin = %g [s]",
                          localWatch.RealTime())
                  << std::endl;
      ...
      if (_time_report)
        std::cout << Form("  CMManagerBase Time Report: IterationEnd = %g [s]",
                          localWatch.RealTime())
                  << std::endl;
      ...
      if (!_merge_till_converge) {
        ...
        if (_debug_mode <= kPerIteration)
          ...
          std::cout
            << "\033[93m  Iterative approach = OFF ... exiting from iteration loop. \033[00m"
            << std::endl;
    ...
    if (_time_report)
      std::cout << Form("  CMManagerBase Time Report: EventEnd = %g [s]", localWatch.RealTime())
                << std::endl;
  }
  ...
  CMManagerBase::ComputePriority(const std::vector<cluster::ClusterParamsAlg>& clusters)
  {
    ...
    for (size_t i = 0; i < clusters.size(); ++i) {
      ...
      if (_priority_algo) { priority = _priority_algo->Priority(clusters.at(c_index)); }
    ...
    if (_time_report)
      std::cout << Form("  CMManagerBase Time Report: ComputePriority = %g [s]",
                        localWatch.RealTime())
                << std::endl;
  }
