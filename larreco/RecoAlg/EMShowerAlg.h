...
public:
  ...
  void FindInitialTrack(const std::map<int, std::vector<art::Ptr<recob::Hit>>>&
...
private:
  ...
  TVector3 Construct3DPoint(art::Ptr<recob::Hit> const& hit1,
                            art::Ptr<recob::Hit> const& hit2) const;

  /// Finds dE/dx for the track given a set of hits
  double FinddEdx(std::vector<art::Ptr<recob::Hit>> const& trackHits,
  ...
  std::map<int, std::vector<art::Ptr<recob::Hit>>> FindShowerStart(
  ...
  std::map<int, std::map<int, bool>> GetPlanePermutations(
  ...
  std::unique_ptr<recob::Track> MakeInitialTrack(
  ...
  std::map<double, int> RelativeWireWidth(
    const std::map<int, std::vector<art::Ptr<recob::Hit>>>& showerHitsMap) const;
