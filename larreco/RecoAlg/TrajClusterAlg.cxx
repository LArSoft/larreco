...
//    if(fStudyMode) outFile.open("quality.txt");
  ...
  void TrajClusterAlg::reconfigure(fhicl::ParameterSet const& pset)
  {
    ...
    std::vector<std::string> specialAlgsVec = pset.get< std::vector<std::string>  >("SpecialAlgs");
    ...
    if(kFlagBigSize != StopFlagNames.size()) throw art::Exception(art::errors::Configuration)<<"kFlagBigSize "<<kFlagBigSize<<" != StopFlagNames size "<<StopFlagNames.size();
    ...
    fUseAlg[kFUH2] = false;
    fUseAlg[kMHOK2] = false;
    fUseAlg[kFTBRevProp] = false;
 ...
 */
  ...
  bool TrajClusterAlg::MergeAndStore(unsigned short itj1, unsigned short itj2)
  {
    ...
    if(PosSep2(tj1.Pts[tj1.EndPt[0]].Pos, tj2.Pts[tj2.EndPt[1]].Pos) <
       PosSep2(tj1.Pts[tj1.EndPt[1]].Pos, tj2.Pts[tj2.EndPt[0]].Pos)) {
      mf::LogWarning("TC")<<"MergeAndStore: Trajectories "<<tj1.ID<<" and "<<tj2.ID<<" are in the wrong order ";
      return false;
    }
      ...
      for(ii = 0; ii < nwires; ++ii) {
        ...
        for(iht = ifirsthit; iht < ilasthit; ++iht) {
          ...
          for(jht = jfirsthit; jht < jlasthit; ++jht) {
            ...
            // BB Dec 1, 2016 angRange > fMaxAngleRange[work.Pass]
            // BB Dec 7, 2016 Bad idea... iHitsInMultiplet.size() > 1 && jHitsInMultiplet.size() > 1 && angRange > fMaxAngleRange[work.Pass]
            ...
            // move the TP position to the hit position but don't mess with the direction Dec 1, 2016
...
//    Refine2DVertices();
  ...
  void TrajClusterAlg::ReversePropagate(Trajectory& tj)
  {
    ...
    unsigned short angRange = AngleRange(tj.Pts[tj.EndPt[0]]);
    if(angRange == fAngleRanges.size() - 1) return;
    ...
    if(prt) {
      mf::LogVerbatim("TC")<<"ReversePropagate: Prepping Tj "<<tj.ID;
//      PrintTrajectory("RPi", tjs, tj, USHRT_MAX);
    }
    ...
    // Dec 5, 2016 - Kind of defeats the purpose of doing this...
//    tjWork.Pts[lastPt].NTPsFit = 3;
    ...
    if(prt) {
      mf::LogVerbatim("TC")<<" ReversePropagate success";
//      PrintTrajectory("RPo", tjs, tj, USHRT_MAX);
    }
...
//        float nTjHits = tmp.size();
  ...
  void TrajClusterAlg::AddLAHits(Trajectory& tj, unsigned short ipt, bool& sigOK)
  {
    ...
    if(angRange != fAngleRanges.size() - 1) {
      mf::LogVerbatim("TC")<<"AddLAHits called with a bad angle range. "<<angRange<<" Don't do this";
      return;
    }
    // and the adjacent wires next in the most likely order only
    // after the first two point have been defined
    if(ipt > 1) {
      if(tp.Dir[0] > 0) {
  ...
  void TrajClusterAlg::AddHits(Trajectory& tj, unsigned short ipt, bool& sigOK)
  {
    ...
    // Call large angle hit finding only on the last range
    unsigned short angleRange = AngleRange(tj.Pts[ipt]);
    if(angleRange == fAngleRanges.size() - 1) {
...
/* Dec 5, 2016. Picks up garbage hits on noisy wires
...
//    for(unsigned short ipt = 0; ipt < tj.Pts.size(); ++ipt)  UnsetUsedHits(tj.Pts[ipt]);
  ...
  //////////////////////////////////////////
  void TrajClusterAlg::FindUseHits2(Trajectory& tj, unsigned short ipt, float maxDelta, bool useChg)
  {
    // Evaluate a different version
    ...
    // This routine shouldn't be called for VLA tjs
    unsigned short angleRange = AngleRange(tp);
    bool isLA = (angleRange == fAngleRanges.size() - 2);
    
    if(prt) {
      mf::LogVerbatim("TC")<<"FUH2:  maxDelta "<<maxDelta<<" useChg requested "<<useChg<<" Norm AveChg "<<(int)tp.AveChg<<" tj.ChgRMS "<<tj.ChgRMS<<" chgPullCut "<<chgPullCut<<" TPHitsRMS "<<(int)TPHitsRMSTick(tjs, tp, kUnusedHits)<<" ExpectedHitsRMS "<<(int)ExpectedHitsRMS(tp)<<" AngleRange "<<angleRange<<" isLA "<<isLA;
    }
    ...
    if(prt) mf::LogVerbatim("TC")<<" nAvailable "<<nAvailable<<" imbest "<<imbest<<" single hit. tp.Delta "<<tp.Delta<<" bestDelta "<<bestDelta<<" path length "<<1 / pathInv;
  ...
  void TrajClusterAlg::FindUseHits(Trajectory& tj, unsigned short ipt, float maxDelta, bool useChg)
  {
    ...
    if(fUseAlg[kFUH2]) {
      FindUseHits2(tj, ipt, maxDelta, useChg);
      return;
    }
    ...
    if(tj.Pass == fMinPts.size() - 1) {
      ...
      if(prt) mf::LogVerbatim("TC")<<"FUH: Using all hits ";
      return;
    }
    ...
    float bestHitChgPull = std::abs(tjs.fHits[bestHit].Integral / tj.AveChg - 1) / tj.ChgRMS;
  ...
  void TrajClusterAlg::DefineHitPos(TrajPoint& tp)
  {
    ...
    if(nused == 1) {
      ...
      // testing. Normalize to 1 WSE path length
      if(fUseAlg[kFUH2]) {
        float pathInv = std::abs(tp.Dir[0]);
        if(pathInv < 0.05) pathInv = 0.05;
        tp.Chg *= pathInv;
...
//      mf::LogVerbatim("TC")<<"vtxPrt set for plane "<<fPlane<<" in SplitTrajCrossingVertices";
  ...
  void TrajClusterAlg::EndMerge()
  {
    ...
    for(unsigned short it1 = 0; it1 < tjs.allTraj.size() - 1; ++it1) {
      ...
      // square it here once
      thetaRMS1 *= thetaRMS1;
      // and divide it by the square of the square root of the path length
      unsigned short e0 = tjs.allTraj[it1].EndPt[0];
      unsigned short e1 = tjs.allTraj[it1].EndPt[1];
      thetaRMS1 /= TrajPointSeparation(tjs.allTraj[it1].Pts[e0], tjs.allTraj[it1].Pts[e1]);
      ...
      for(unsigned short end1 = 0; end1 < 2; ++end1) {
        ...
        for(unsigned short it2 = 0; it2 < tjs.allTraj.size(); ++it2) {
          ...
          if(PosSep2(tjs.allTraj[it1].Pts[tjs.allTraj[it1].EndPt[end1]].Pos, tjs.allTraj[it2].Pts[tjs.allTraj[it2].EndPt[end1]].Pos) <
             PosSep2(tjs.allTraj[it1].Pts[tjs.allTraj[it1].EndPt[end1]].Pos, tjs.allTraj[it2].Pts[tjs.allTraj[it2].EndPt[end2]].Pos)) {
            continue;
          }
          // count the number of dead wires between them
          float dwc = DeadWireCount(tjs, tp1, tp2);
          ...
          if(mrgPrt && abs(ptSep) < 20) mf::LogVerbatim("TC")<<" EM "<<tjs.allTraj[it1].ID<<"_"<<end1<<"-"<<tjs.allTraj[it2].ID<<"_"<<end2<<" tp1-tp2 "<<PrintPos(tjs, tp1)<<"-"<<PrintPos(tjs, tp2)<<" ptSep "<<ptSep<<" min "<<minPtSep<<" max "<<fMaxWireSkipNoSignal<<" isLA? "<<isLA;
          // maximum overlap && maximum missed (live) wires
          if(ptSep < minPtSep || ptSep > fMaxWireSkipNoSignal) continue;
          ...
          float tp2ang = tp2.Ang;
          if(tjs.allTraj[it2].Pts.size() > 5) {
            if(end2 == 0) {
              unsigned short usePt = tjs.allTraj[it2].EndPt[end2] + 2;
          ...
          if(mrgPrt)  mf::LogVerbatim("TC")<<"  tp1ang "<<tp1ang<<" tp2ang "<<tp2ang<<" dang "<<dang<<" isLA? "<<isLA;
          // Get the MCS angle expected for the highest MCSMom trajectory
          float thetaRMS2 = MCSThetaRMS(tjs, tjs.allTraj[it2]);
          unsigned short e0 = tjs.allTraj[it2].EndPt[0];
          unsigned short e1 = tjs.allTraj[it2].EndPt[1];
          ...
          if(isLA) {
            ...
            if(mrgPrt)  mf::LogVerbatim("TC")<<"  isLA TrajTrajDOCA doca "<<doca<<" ipt1 "<<ipt1<<" ipt2 "<<ipt2;
          } else {
            // small angle
          ...
          bool doMerge = doca < docaCut && dang < dangCut;
          bool showerTjs = tjs.allTraj[it1].PDGCode == 12 || tjs.allTraj[it2].PDGCode == 2;
          ...
          if(doMerge) {
            ...
            if(mrgPrt) {
              mf::LogVerbatim("TC")<<"  Merge ";
              PrintTrajectory("EM", tjs, tjs.allTraj[it1], USHRT_MAX);
              PrintTrajectory("EM", tjs, tjs.allTraj[it2], USHRT_MAX);
            }
            if(fQuitAlg) return;
            bool didMerge;
            if(end1 == 1) {
            ...
            // assume that both are long
            float sepCut = fVertex2DCuts[2];
            if(tj1Short || tj2Short) sepCut = fVertex2DCuts[1];
            float wint, tint;
            TrajIntersection(tp1, tp2, wint, tint);
            if(mrgPrt) mf::LogVerbatim("TC")<<"  Try a vertex at wint:tint "<<(int)wint<<":"<<(int)tint<<" ticks "<<(int)(tint/tjs.UnitsPerTick);
            ...
            if(!FitVertex(tjs, aVtx, fVertex2DCuts, mrgPrt)) {
              ...
              tjs.allTraj[it1].AlgMod[kEndMerge] = false;
              tjs.allTraj[it2].AlgMod[kEndMerge] = false;
              continue;
            }
            tjs.vtx.push_back(aVtx);
            AttachAnyTrajToVertex(tjs, newVtxID - 1, fVertex2DCuts, mrgPrt);
            if(mrgPrt) mf::LogVerbatim("TC")<<"  Made vertex ID "<<newVtxID;
          } // create a vertex
...
/*
          float intChg = 0;
          unsigned short cnt = 0;
          for(unsigned short ipt = intPt2 - 2; ipt < intPt2 + 2; ++ipt) {
            if(tjs.allTraj[it2].Pts[ipt].Chg == 0) continue;
            intChg += tjs.allTraj[it2].Pts[ipt].Chg;
            ++cnt;
          } // ipt
          if(cnt > 0) {
            intChg /= cnt;
            float chgPull = (intChg - tjs.allTraj[it2].AveChg) / tjs.allTraj[it2].ChgRMS;
            if(vtxPrt) mf::LogVerbatim("TC")<<" chgPull at intersection point "<<chgPull;
            if(chgPull < 10) continue;
          }
*/
    ...
    for(unsigned short it1 = 0; it1 < tjs.allTraj.size(); ++it1) {
      ...
      for(unsigned short end1 = 0; end1 < 2; ++end1) {
        ...
        for(unsigned short it2 = 0; it2 < tjs.allTraj.size(); ++it2) {
          ...
          unsigned short closePt2 = 0;
          TrajPointTrajDOCA(tjs, tjs.allTraj[it1].Pts[endPt1], tjs.allTraj[it2], closePt2, doca);
//          std::cout<<"FHV "<<tjs.allTraj[it1].ID<<"  "<<tjs.allTraj[it2].ID<<" doca "<<doca<<" tj2.EndPt[0] "<<tjs.allTraj[it2].EndPt[0]<<" closePt2 "<<closePt2<<" tjs.allTraj[it2].EndPt[1] "<<tjs.allTraj[it2].EndPt[1]<<" doca "<<doca<<"\n";
          if(doca == minDOCA) continue;
          // ensure that the closest point is not near an end
...
//      std::cout<<"CIC new vtx "<<tjs.vtx[ivx].ID<<"\n";
  ...
  void TrajClusterAlg::StepCrawl(Trajectory& tj)
  {
    ...
    for(unsigned short step = 1; step < 10000; ++step) {
      ...
      if(angRange < fAngleRanges.size() - 1) stepSize = std::abs(1/ltp.Dir[0]);
...
//        tj.MCSMom = MCSMom(tjs, tj);
      ...
      if(tj.Pts[lastPt].Hits.empty()) {
        ...
        if(lastPt > 0) {
          ...
          if(angRange > 0 && nMissedSteps > 4 && !SignalAtTp(ltp)) {
  ...
  {
    // See if trajectory tj shares many hits with another trajectory and if so merges them.
    ...
    hitCnt /= 2;
    unsigned short oldTj = USHRT_MAX;
    ...
    for(unsigned short ipt = 0; ipt < tjs.allTraj[oldTj].Pts.size(); ++ipt) {
      TrajPoint& tp = tjs.allTraj[oldTj].Pts[ipt];
      bool didIt = false;
      for(unsigned short ii = 0; ii < tp.Hits.size(); ++ii) {
        unsigned int iht = tp.Hits[ii];
        if(tjs.fHits[iht].InTraj == tj.ID) {
          tp.UseHit[ii] = true;
          tjs.fHits[iht].InTraj = tjs.allTraj[oldTj].ID;
          didIt = true;
        }
      } // ii
      if(didIt) {
        ...
        DefineHitPos(tp);
        TrajPoint tpFit;
        FitTraj(tjs.allTraj[oldTj], ipt, tp.NTPsFit, -1, tpFit);
        tp = tpFit;
        if(prt) PrintTrajectory("IG", tjs, tjs.allTraj[oldTj], ipt); 
      }
    } // tp
  ...
  void TrajClusterAlg::CheckTraj(Trajectory& tj)
  {
    ...
    // See if this is a ghost trajectory
    if(IsGhost(tj)) {
      if(prt) mf::LogVerbatim("TC")<<" CT: Ghost trajectory ";
      fGoodTraj = false;
      return;
    }
    ...
    if(isVLA && HasDuplicateHits(tjs, tj, prt)) {
      fGoodTraj = false;
      return;
    }
    ...
    if(fUseAlg[kTrimHits] && !tj.StopFlag[1][kBragg]) {
      // First ensure that there are at least two points with charge at the end
      while(NumPtsWithCharge(tjs, tj, false) > fMinPts[tj.Pass]) {
        unsigned short lastPt = tj.EndPt[1];
      ...
      unsigned short dwc = DeadWireCount(tjs, tj.Pts[tj.EndPt[1]-1], tj.Pts[tj.EndPt[1]]);
      if(prt) mf::LogVerbatim("TC")<<"kTrimHits: dead wire count "<<dwc;
      while(dwc > 0 && NumPtsWithCharge(tjs, tj, false) > fMinPts[tj.Pass]) {
        ...
        tj.Pts.resize(tj.EndPt[1]+1);
        SetEndPoints(tjs, tj);
        dwc = DeadWireCount(tjs, tj.Pts[tj.EndPt[1]-1], tj.Pts[tj.EndPt[1]]);
      ...
      dwc = DeadWireCount(tjs, tj.Pts[tj.EndPt[0]], tj.Pts[tj.EndPt[1]]);
      float nPtsWithCharge = NumPtsWithCharge(tjs, tj, false);
      ...
      while(ptFrac < 0.7 && nPts > 1) {
        ...
        dwc = DeadWireCount(tjs, tj.Pts[tj.EndPt[0]], tj.Pts[tj.EndPt[1]]);
        nPtsWithCharge = NumPtsWithCharge(tjs, tj, false);
        if(nPtsWithCharge < fMinPts[tj.Pass]) return;
        ptFrac = (nPtsWithCharge + dwc) /(nPts + dwc);
        tj.AlgMod[kTrimHits] = true;
        // there must be hits here since we are masking them off
        tj.StopFlag[1][kSignal] = true;
      } // ptFrac < 0.7 && nPts > 1
  ...
  ////////////////////////////////////////////////
  void TrajClusterAlg::ChkTrajBegin(Trajectory& tj)
  {
    // Check the environment in the vicinity of the start and define StopFlag[0][kSignal]
    if(tj.EndPt[0] != 0) {
      tj.StopFlag[0][kSignal] = true;
      return;
    }
    // check one wire on the other side of EndPt[0] to see if it is clean.
    TrajPoint tp = tj.Pts[0];
    // Move the TP "backwards"
    unsigned short angRange = AngleRange(tp);
    double stepSize = fVLAStepSize;
    if(angRange < fAngleRanges.size() - 1) stepSize = std::abs(1/tp.Dir[0]);
    tp.Pos[0] += tp.Dir[0] * stepSize * tj.StepDir;
    tp.Pos[1] += tp.Dir[1] * stepSize * tj.StepDir;
    if(SignalAtPos(tp.Pos[0], tp.Pos[1], tj.CTP)) tj.StopFlag[0][kSignal] = true;
    if(prt) mf::LogVerbatim("TC")<<"CTB: Signal at pos "<<PrintPos(tjs, tp)<<"? "<<tj.StopFlag[0][kSignal];
  ...
  void TrajClusterAlg::FixTrajBegin(Trajectory& tj)
  {
    ...
    // don't do anything if the environment is clean at the beginning
    if(!tj.StopFlag[0][kSignal]) return;
...
/* Dec 3, 2016
 // Try this out. Drop the TPs with charge and re-find them unless they are large angle
      if(tp.Chg > 0 && AngleRange(tp) == 0) {
        float chgIn = tp.Chg;
        // tj.StopsAtEnd[0] && ipt < firstPt + 6 && tp.Hits.size() > 1
        if(tj.StopFlag[0][kBragg] && ipt < firstPt + 6 && tp.Hits.size() > 1) {
...
//    if(tj.AlgMod[kGottaKink]) return;
  ...
  bool TrajClusterAlg::MaskedHitsOK(Trajectory& tj)
  {
    ...
    if(!tj.AlgMod[kUnMaskHits] && lastPt > 10 && nMasked > 4 && nMasked == nClose) {
      ...
      SetEndPoints(tjs, tj);
      // try to fit them all
      tj.Pts[lastPt].NTPsFit = NumPtsWithCharge(tjs, tj, false);
    ...
    ////////////////////////////////////////////////
  bool TrajClusterAlg::MaskedHitsOK2(Trajectory& tj)
  {
...
/*
  ////////////////////////////////////////////////
  void TrajClusterAlg::WatchHit(std::string someText, int tjID)
  {
    // a temp routine to watch when inTraj changes.
    if(tjs.fHits[1821].InTraj != watchInTraj) {
      std::cout<<someText<<" "<<tjID<<" Change "<<tjs.fHits[1821].InTraj<<"\n";
      watchInTraj = tjs.fHits[1821].InTraj;
    }
  }
*/
  ////////////////////////////////////////////////
  ...
  void TrajClusterAlg::UpdateTraj(Trajectory& tj)
  {
    ...
    // Restrict the number of fitted hits if this is a low momentum trajectory (using the last 10 points)
    // Dec 5, 2016. use MCSMom calculated from the last ~10 points
...
//    if(AngleRange(lastTP) == fAngleRanges.size() - 1) lastTP.NTPsFit = fMinPtsFit[tj.Pass];
    ...
    if(lastTP.FitChi > 1.5 && tj.Pts.size() > 6) {
      ...
      if(ndead > 5) {
        ...
        // Dec 5, 2016. Don't mask hits when doing RevProp. Reduce NTPSFit instead
    ...
    if(fMaskedLastTP) {
      ...
      while(lastTP.FitChi > 1.5 && lastTP.NTPsFit > minPtsFit) {
        ...
        if(lastTP.FitChi > prevChi) {
          ...
          MaskBadTPs(tj, 1.5);
          // Added Dec 6 for a LA trajectory
          ++ntry;
          if(ntry == 2) break;
        }
