    ...
    for(unsigned short pass = 0; pass < fMinPtsFit.size(); ++pass) {
      ...
      for(ii = 0; ii < nwires; ++ii) {
        ...
        for(iht = ifirsthit; iht < ilasthit; ++iht) {
          prt = (iht == debug.Hit);
          if(prt) didPrt = true;
          if(prt)  mf::LogVerbatim("TC")<<"+++++++ Pass "<<fPass<<" Found debug hit "<<fPlane<<":"<<PrintHit(tjs.fHits[iht])<<" tjs.inTraj "<<tjs.inTraj[iht];
...
//              HitMultipletPosition(jht, toTick, deltaRms, jqtot);
            ...
            Trajectory work;
            StartTraj(work, fromWire, fromTick, toWire, toTick, fCTP);
...
//        if(!IsLargeAngle(tj.Pts[ipt])) continue;
  ...
  void TrajClusterAlg::ReversePropagate(Trajectory& tj)
  {
    ...
    mf::LogVerbatim("TC")<<" wire0 "<<wire0<<" nextWire "<<nextWire;
    ...
    // find close hits near this position. Note that FindCloseHits doesn't
    // check for proximity using delta Time, just DOCA.
    float maxDelta = 10 * tj.Pts[tj.EndPt[1]].DeltaRMS;
    tp.Hits = FindCloseHits(tjs, tp, maxDelta, false);
    ...
    if(nAvailable == 0) {
      if(prt) mf::LogVerbatim("TC")<<" No available hits";
      return;
    }
    DefineHitPos(tp);
    ...
    // append the TP
    tjWork.Pts.push_back(tp);
    SetEndPoints(tjs, tjWork);
    UpdateTraj(tjWork);
    if(!fUpdateTrajOK || fQuitAlg) {
      if(prt) mf::LogVerbatim("TC")<<" UpdateTraj failed";
      return;
    }
    unsigned short ipt = tjWork.Pts.size() - 1;
    if(prt) PrintTrajPoint("RP", tjs, ipt, tjWork.StepDir, tjWork.Pass, tjWork.Pts[ipt]);
    ReverseTraj(tjs, tjWork);
    tj = tjWork;
...
//              if(prt) mf::LogVerbatim("TC")<<" kwire "<<kwire<<" thits size "<<tHits.size();
  ...
  void TrajClusterAlg::MakeJunkTraj(std::vector<unsigned int> tHits, unsigned short& newTjIndex)
  {
    ...
    Trajectory work;
    StartTraj(work, tHits[0], tHits[tHits.size()-1]);
...
//    TrajPoint ltp;
  ...
  void TrajClusterAlg::AddHits(Trajectory& tj, unsigned short ipt, bool& sigOK)
  {
    ...
    // Assume a signal exists on a dead wire
    if(tjs.WireHitRange[fPlane][wire].first == -1) sigOK = true;
    if(tjs.WireHitRange[fPlane][wire].first < 0) return;
    unsigned int firstHit = (unsigned int)tjs.WireHitRange[fPlane][wire].first;
    unsigned int lastHit = (unsigned int)tjs.WireHitRange[fPlane][wire].second;
    float fwire = wire;
    for(unsigned int iht = firstHit; iht < lastHit; ++iht) {
...
//        if(!isLA && lastPt == 2) return;
      ...
      if(killPts == 0 &&  tj.Pts[lastPt].FitChi > fMaxChi && tj.PDGCode != 13) {
        ...
        fGoodTraj = (NumPtsWithCharge(tj, true) > fMinPtsFit[fPass]);
        return;
      }
...
//    bool sortaLargeAngle = (std::abs(tj.Pts[ii].Dir[0]) < fLargeAngle + 0.1);
  ...
  bool TrajClusterAlg::MaskedHitsOK(Trajectory& tj)
  {
    ...
    for(unsigned short ii = 1; ii < tj.Pts.size(); ++ii) {
      ...
      if(tj.Pts[ipt].Chg > 0) break;
      ++nMasked;
      nClose += tj.Pts[ipt].Hits.size();
    } // ii
    
    if(nMasked == 0) return true;
    
    if(prt) mf::LogVerbatim("TC")<<"MaskedHitsOK:  nMasked "<<nMasked<<" nClose "<<nClose<<" fMaxWireSkipWithSignal "<<fMaxWireSkipWithSignal;
...
//      InterceptError = sqrt(varnce * sumx2 / delta);
  ...
  ////////////////////////////////////////////////
  void TrajClusterAlg::StartTraj(Trajectory& tj, unsigned int fromHit, unsigned int toHit)
  {
    ...
    StartTraj(tj, fromWire, fromTick, toWire, toTick, tCTP);
  } // StartTraj

  ////////////////////////////////////////////////
  void TrajClusterAlg::StartTraj(Trajectory& tj, float fromWire, float fromTick, float toWire, float toTick, CTP_t tCTP)
  {
...
//      std::cout<<"StoreTraj AveChg at end is 0 on Traj ID "<<tj.ID<<"\n";
    ...
    tj.EndTP[0].AveChg = tj.Pts[tj.EndPt[0]].AveChg;
    tj.EndTP[1].AveChg = tj.Pts[tj.EndPt[1]].AveChg;
