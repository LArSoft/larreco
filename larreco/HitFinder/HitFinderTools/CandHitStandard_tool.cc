...
namespace reco_tool {
  ...
  private:
    ...
    const float
      fNextADCThreshold; ///< minimum difference between adjacent ADCs to define peak edges
  ...
  CandHitStandard::CandHitStandard(const fhicl::ParameterSet& pset)
    ...
    , fNextADCThreshold(pset.get<float>("NextADCThreshold", 0.0))
  {}
    ...
    if (std::distance(startItr, stopItr) > 4) {
      ...
      if (maxValue > fRoiThreshold) {
        ...
        while (lastItr != stopItr - 1) {
          ...
          // Check both sides of lastItr and look for min/inflection point
