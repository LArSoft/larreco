...
#include <iostream> #include "BoundingBox.h" #include <stdio.h> int main(int argc, char** argv)
{
  int x = 0;
  RStarBoundingBox<1> b[20]; // line tests
  ...
  b[x++].edges[0] = std::make_pair(0, 1);
  b[x++].edges[0] = std::make_pair(0, 1); // point tests
  ...
  for (int i = 0; i < x / 2; i++) {
    ...
    printf("%s %s %s\n",
           b[i1].ToString().c_str(),
           b[i1].encloses(b[i2]) ? "encloses" : "does not enclose",
           b[i2].ToString().c_str());
    printf("%s %s %s\n",
           b[i2].ToString().c_str(),
           b[i2].encloses(b[i1]) ? "encloses" : "does not enclose",
           b[i1].ToString().c_str());
    printf("%s %s %s (%f)\n",
           b[i1].ToString().c_str(),
           b[i1].overlaps(b[i2]) ? "overlaps" : "does not overlap",
           b[i2].ToString().c_str(),
           b[i1].overlap(b[i2]));
    printf("%s %s %s (%f)\n\n",
           b[i2].ToString().c_str(),
           b[i2].overlaps(b[i1]) ? "overlaps" : "does not overlap",
           b[i1].ToString().c_str(),
           b[i2].overlap(b[i1]));
  }
}