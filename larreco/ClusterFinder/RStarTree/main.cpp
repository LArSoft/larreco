...
struct Visitor {
  ...
  void operator()(const RTree::Leaf* const leaf)
  {
    ...
    std::cout << "#" << count << ": visited " << leaf->leaf << " with bound "
...
#define nodes 20000
  ...
  std::cout << "AcceptAny: " << x.count << " nodes visited (" << tree.GetSize() << " nodes in tree)"
            << std::endl;

  std::cout << "Searching in " << bound.ToString() << std::endl;
  x = tree.Query(RTree::AcceptEnclosing(bound), Visitor());
  std::cout << "Visited " << x.count << " nodes (" << tree.GetSize() << " nodes in tree)"
            << std::endl;
