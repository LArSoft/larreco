...
public:
  ...
  // destructor
  ~RStarTree() { Remove(AcceptAny(), RemoveLeaf()); }
  ...
  void RemoveBoundedArea(const BoundingBox& bound) { Remove(AcceptEnclosing(bound), RemoveLeaf()); }
...
protected:
  ...
  Node* ChooseSubtree(Node* node, const BoundingBox* bound)
  {
    ...
    if (static_cast<Node*>(node->items[0])->hasLeaves) {
      ...
      if (max_child_items > (RTREE_CHOOSE_SUBTREE_P * 2) / 3 &&
        ...
        std::partial_sort(node->items.begin(),
        ...
        return static_cast<Node*>(
          *std::min_element(node->items.begin(),
                            node->items.begin() + RTREE_CHOOSE_SUBTREE_P,
  ...
  Node* OverflowTreatment(Node* level, bool firstInsert)
  {
    ...
    if (level == m_root) {
      ...
      newRoot->bound.reset();
      for_each(newRoot->items.begin(),
               newRoot->items.end(),
               StretchBoundingBox<BoundedItem>(&newRoot->bound));
  ...
  Node* Split(Node* node)
  {
    ...
    for (std::size_t axis = 0; axis < dimensions; axis++) {
      ...
      for (std::size_t edge = 0; edge < 2; edge++) {
        ...
        if (edge == 0)
          std::sort(
            node->items.begin(), node->items.end(), SortBoundedItemsByFirstEdge<BoundedItem>(axis));
        else
          std::sort(node->items.begin(),
                    node->items.end(),
                    SortBoundedItemsBySecondEdge<BoundedItem>(axis));
        ...
        for (std::size_t k = 0; k < distribution_count; k++) {
          ...
          R1.reset();
          for_each(node->items.begin(),
                   node->items.begin() + (min_child_items + k),
                   StretchBoundingBox<BoundedItem>(&R1));

          // then do the same for R2
          R2.reset();
          for_each(node->items.begin() + (min_child_items + k + 1),
                   node->items.end(),
    ...
    if (split_edge == 0)
      std::sort(node->items.begin(),
                node->items.end(),
                SortBoundedItemsByFirstEdge<BoundedItem>(split_axis));

    // only reinsert the sort key if we have to
    else if (split_axis != dimensions - 1)
      std::sort(node->items.begin(),
                node->items.end(),
                SortBoundedItemsBySecondEdge<BoundedItem>(split_axis));
    ...
    node->bound.reset();
    std::for_each(
      node->items.begin(), node->items.end(), StretchBoundingBox<BoundedItem>(&node->bound));

    newNode->bound.reset();
    std::for_each(newNode->items.begin(),
                  newNode->items.end(),
                  StretchBoundingBox<BoundedItem>(&newNode->bound));
  ...
  void Reinsert(Node* node)
  {
    ...
    const std::size_t n_items = node->items.size();
    const std::size_t p = (std::size_t)((double)n_items * RTREE_REINSERT_P) > 0 ?
    ...
    for (typename std::vector<BoundedItem*>::iterator it = removed_items.begin();
         it != removed_items.end();
         it++)
    ...
    void operator()(BoundedItem* item)
    {
      ...
      if (visitor.ContinueVisiting && accept(node)) {
        if (node->hasLeaves)
          for_each(node->items.begin(),
                   node->items.end(),
                   VisitFunctor<Acceptor, Visitor>(accept, visitor));
        else
  ...
  struct RemoveFunctor : std::unary_function<const BoundedItem*, bool> {
    ...
    // are put into
    explicit RemoveFunctor(const Acceptor& na,
                           LeafRemover& lr,
                           std::list<Leaf*>* ir,
                           std::size_t* size)
    ...
    bool operator()(BoundedItem* item, bool isRoot = false)
    {
      ...
      if (accept(node)) {
        ...
        if (node->hasLeaves)
          node->items.erase(
            std::remove_if(node->items.begin(),
                           node->items.end(),
                           RemoveLeafFunctor<Acceptor, LeafRemover>(accept, remove, m_size)),
            node->items.end());
        else
          node->items.erase(std::remove_if(node->items.begin(), node->items.end(), *this),
                            node->items.end());
