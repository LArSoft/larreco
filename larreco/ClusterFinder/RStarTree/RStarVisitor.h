...
struct RStarAcceptOverlapping {
  ...
  bool operator()(const Node* const node) const { return m_bound.overlaps(node->bound); }

  bool operator()(const Leaf* const leaf) const { return m_bound.overlaps(leaf->bound); }
...
struct RStarAcceptEnclosing {
  ...
  bool operator()(const Leaf* const leaf) const { return m_bound.encloses(leaf->bound); }
...
struct RStarRemoveSpecificLeaf {
  ...
  explicit RStarRemoveSpecificLeaf(const typename Leaf::leaf_type& leaf,
  ...
  bool operator()(const Leaf* const leaf) const
  {
    if (ContinueVisiting && m_leaf == leaf->leaf) {
      if (!m_remove_duplicates) ContinueVisiting = false;
      return true;
    }
    return false;
  }
