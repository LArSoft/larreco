...
void cluster::BlurredClustering::produce(art::Event& evt)
{
  ...
  for (auto const& hitToCluster : hitsToCluster) {
    ...
    auto const plane = wireID.Plane;
    auto const tpc = fGlobalTPCRecon ? wireID.TPC%2 : wireID.TPC;
    planeToHits[std::make_pair(plane, tpc)].push_back(hitToCluster);
  }
  ...
  // Loop over views
  for (auto const& plane : planeToHits) {

    std::vector<art::PtrVector<recob::Hit>> finalClusters;

    // Implement the algorithm
    if (plane.second.size() >= fBlurredClusteringAlg.GetMinSize()) {

      // Convert hit map to TH2 histogram and blur it
      std::vector<std::vector<double>> image = fBlurredClusteringAlg.ConvertRecobHitsToVector(plane.second);
      std::vector<std::vector<double>> blurred = fBlurredClusteringAlg.GaussianBlur(image);

       // Find clusters in histogram
      std::vector<std::vector<unsigned int>> allClusterBins; // Vector of clusters (clusters are vectors of hits)
      auto const numClusters = fBlurredClusteringAlg.FindClusters(blurred, allClusterBins);
      ...
      if (fMergeClusters) {
        auto const numMergedClusters = fMergeClusterAlg.MergeClusters(planeClusters, finalClusters);
      ...
      if (fCreateDebugPDF) {
        ...
        fBlurredClusteringAlg.SaveImage(imageHist, 1, plane.first.second, plane.first.first);
        fBlurredClusteringAlg.SaveImage(blurredHist, 2, plane.first.second, plane.first.first);
        fBlurredClusteringAlg.SaveImage(blurredHist, allClusterBins, 3, plane.first.second, plane.first.first);
        fBlurredClusteringAlg.SaveImage(imageHist, finalClusters, 4, plane.first.second, plane.first.first);
        imageHist->Delete();
        blurredHist->Delete();
      }
