...
namespace shwf {
  ...
  private:
    ...
    void LongTransEnergy(unsigned int set,
                         std::vector<art::Ptr<recob::Hit>> hitlist,
                         bool isData = false); //Longtudinal
  ...
  ShowerReco::produce(art::Event& evt)
  {
    ...
    std::unique_ptr<art::Assns<anab::Calorimetry, recob::Shower>> calassn(
      new art::Assns<anab::Calorimetry, recob::Shower>);
    ...
    for (size_t iClustSet = 0; iClustSet < clusterAssociationHandle->size(); iClustSet++) {
      ...
      try {
        ...
        xyz_vertex_fit[2] = z;
        xyz_vertex_fit[0] =
          (fTime_vertex[bp1] - detprop->TriggerOffset()) * fDriftVelocity * fTimeTick +
          position[0][0];
      ...
      calorimetrycol->push_back(anab::Calorimetry(
  ...
  //------------------------------------------------------------------------------
  void
  ShowerReco::LongTransEnergy(unsigned int set,
                              std::vector<art::Ptr<recob::Hit>> hitlist,
                              bool /*isData*/)
  {
    ...
    double totCnrg = 0,
           totCnrg_corr = 0; //, totNewCnrg=0 ; // tot enegry of the shower in collection
                             //   art::ServiceHandle<geo::Geometry const> geom;
    //   auto const* detprop = lar::providerFrom<detinfo::DetectorPropertiesService>();
    ...
    for (art::PtrVector<recob::Hit>::const_iterator hitIter = hitlist.begin();
         hitIter != hitlist.end();
         hitIter++) {
      ...
      if ((wdist < fcalodEdxlength) && (wdist > 0.2)) {
        ...
        if (wdist < fdEdxlength &&
            ((direction == 1 &&
              wire > fWire_vertex[plane]) //take no hits before vertex (depending on direction)
    ...
    for (art::PtrVector<recob::Hit>::const_iterator hitIter = hitlist.begin();
      ...
      if (fUseArea) { dEdx = calalg.dEdx_AREA((*hitIter), newpitch); }
      else //this will hopefully go away, once all of the calibration factors are calculated.
      {
