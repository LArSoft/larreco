...
namespace ShowerRecoTools {
                                       ...
                                       reco::shower::ShowerElementHolder& ShowerEleHolder)
  {
    ...
    if (!Event.getByLabel(fPFParticleModuleLabel, pfpHandle)) {
      throw cet::exception("ShowerPCADirection")
    ...
    if (!Event.getByLabel(fPFParticleModuleLabel, spHandle)) {
      throw cet::exception("ShowerPCADirection") << "Could not configure the spacepoint handle. "
                                                    "Something is configured incorrectly. Stopping";
      return 1;
    }
    ...
    if (fUseStartPosition) {
      if (!ShowerEleHolder.CheckElement(fShowerStartPositionInputLabel)) {
        throw cet::exception("ShowerPCADirection")
          << "fUseStartPosition is true but start position is not set. Stopping.";
        return 1;
      }
  ...
  double
  ShowerPCADirection::RMSShowerGradient(std::vector<art::Ptr<recob::SpacePoint>>& sps,
                                        TVector3& ShowerCentre,
                                        TVector3& Direction)
  {
