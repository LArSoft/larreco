...
namespace ShowerRecoTools {
                                           ...
                                           reco::shower::ShowerElementHolder& ShowerEleHolder)
  {
    ...
    if (ShowerEleHolder.CheckElement(fTrueParticleInputLabel)) {
      ...
      std::map<int, std::vector<int>> showersMothers =
        fTRACSCheatingAlg.GetTrueChain(trueParticles);

      //Get the clusters
      art::Handle<std::vector<recob::Cluster>> clusHandle;
      if (!Event.getByLabel(fPFParticleModuleLabel, clusHandle)) {
        throw cet::exception("ShowerDirectionCheater")
          << "Could not get the pandora clusters. Something is not cofingured coreectly Please "
             "give the correct pandoa module label. Stopping";
        return 1;
      }
  ...
  double
  ShowerDirectionCheater::RMSShowerGradient(std::vector<art::Ptr<recob::SpacePoint>>& sps,
                                            TVector3& ShowerCentre,
                                            TVector3& Direction)
  {
    ...
    double minProj =
      IShowerTool::GetTRACSAlg().SpacePointProjection(sps[0], ShowerCentre, Direction);
    double maxProj =
      IShowerTool::GetTRACSAlg().SpacePointProjection(sps[sps.size() - 1], ShowerCentre, Direction);
    ...
    for (auto const& sp : sps) {
      ...
      double len_perp =
        IShowerTool::GetTRACSAlg().SpacePointPerpendiular(sp, ShowerCentre, Direction, len);
  ...
  double
  ShowerDirectionCheater::CalculateRMS(std::vector<float> perps)
  {
    int counter = 0;
    double sum = 0;
