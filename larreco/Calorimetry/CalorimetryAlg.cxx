...
namespace calo {
  ...
  //--------------------------------------------------------------------
  void
  CalorimetryAlg::reconfigure(const Config& config)
  {
  ...
  // ----------------------------------------------------------------------------------//
  double
  CalorimetryAlg::dEdx_AMP(art::Ptr<recob::Hit> hit, double pitch, double T0) const
  {
  ...
  // ----------------------------------------------------------------------------------//
  double
  CalorimetryAlg::dEdx_AMP(recob::Hit const& hit, double pitch, double T0) const
  {
    return dEdx_AMP(hit.PeakAmplitude() / pitch, hit.PeakTime(), hit.WireID().Plane, T0);
  }
  ...
  // ----------------------------------------------------------------------------------//
  double
  CalorimetryAlg::dEdx_AMP(double dQ, double time, double pitch, unsigned int plane, double T0)
    const
  {
  ...
  // ----------------------------------------------------------------------------------//
  double
  CalorimetryAlg::dEdx_AMP(double dQdx, double time, unsigned int plane, double T0) const
  {
    double fADCtoEl = 1.;
  ...
  // ----------------------------------------------------------------------------------//
  double
  CalorimetryAlg::dEdx_AREA(art::Ptr<recob::Hit> hit, double pitch, double T0) const
  {
  ...
  // ----------------------------------------------------------------------------------//
  double
  CalorimetryAlg::dEdx_AREA(recob::Hit const& hit, double pitch, double T0) const
  {
    return dEdx_AREA(hit.Integral() / pitch, hit.PeakTime(), hit.WireID().Plane, T0);
  }
  ...
  // ----------------------------------------------------------------------------------//
  double
  CalorimetryAlg::dEdx_AREA(double dQ, double time, double pitch, unsigned int plane, double T0)
    const
  {
    double dQdx = dQ / pitch; // in ADC/cm
  ...
  // ----------------------------------------------------------------------------------//
  double
  CalorimetryAlg::dEdx_AREA(double dQdx, double time, unsigned int plane, double T0) const
  {
    double fADCtoEl = 1.;
  ...
  double
  CalorimetryAlg::dEdx_from_dQdx_e(double dQdx_e, double time, double T0) const
  {
    if (fDoLifeTimeCorrection)
      dQdx_e *= LifetimeCorrection(time, T0); // Lifetime Correction (dQdx_e in e/cm)
    if (fUseModBox) { return detprop->ModBoxCorrection(dQdx_e); }
    else {
  ...
  // ----------------------------------------------------------------------------------//
  double
  calo::CalorimetryAlg::LifetimeCorrection(double time, double T0) const
  {
    float t = time;
    ...
    if (fLifeTimeForm == 0) {
      ...
      const lariov::ElectronLifetimeProvider& elifetime_provider =
      ...
      throw cet::exception("CalorimetryAlg")
        << "Unknow CaloLifeTimeForm " << fLifeTimeForm << std::endl;
    }
  }
